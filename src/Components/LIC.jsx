...
const plans = [
  {
    ...
    features: [
      ...
      // "📲 First day insurance coverage starts immediately",
    ],
...
const LIC = () => {
  return (
    <div>
      ...
      >
        <div className="max-w-screen-lg overflow-visible mx-auto p-4 flex flex-col justify-center w-full h-full text-left">
          <div>
          <p className="text-2xl md:text-4xl font-bold p-2 border-b-4 border-gray-500 inline">
            LIC Plans
          </p>
          </div>
          <div className="mt-10 px-4 md:px-0 relative overflow-visible">
          <Swiper
...
>
    ...
    <SwiperSlide key={index}>
      <div className="relative flex flex-col lg:flex-row items-center gap-8 pb-10 lg:pl-11">
        <img
          src={plan.image}
          alt={`Plan ${index + 1}`}
          className="w-full md:w-[400px] h-[350px] object-cover rounded-xl shadow-md"
        />
        <div className="text-gray-700 space-y-4 w-full">
          ...
          <a
            ...
            className="inline-block mt-4 px-6 py-3 bg-yellow-500 hover:bg-yellow-600 text-white text-lg rounded-full transition-all duration-300 shadow-md"
          >
  ...
  {/* Custom Nav Buttons with Tailwind – Hidden on Mobile */}
